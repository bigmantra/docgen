...
test.describe('docgenButton Component - Real Backend Tests', () => {
  // These tests only run when TEST_MODE_DISABLED=true
  // They validate the full integration: Salesforce → AAD OAuth → Node.js Backend → PDF Generation

  test('generates PDF successfully with real backend', async ({ salesforce }) => {
    // Only run when TEST_MODE_DISABLED
